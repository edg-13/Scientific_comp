/*****************************************************************
 * This programme is used to simulate the spread of a chemical in 
 * the presence of active agents in 2 spatial dimensions, with
 * given boundary conditions.
 * Prior to compilation execute following lines on nenneke:
 * module purge
 * module load intel impi imkl
 * Then:
 * Compile:  gcc -o assign5 assign5_v2.c -lm -lmkl -liomp5
 * Run: ./assign5
 * ****************************************************************/
#include <stdlib.h>
#include <stdio.h>
...
long indx2(long k, long m, long M);
...
int main() {
    ...
    *******************************/
    ...
    //Size of bmat depends on one spatial dimension so dimension with smallest number of grid points is chosen
    if(N_x<N_y){
        ...
        /* Default matrix with no boundary conditions using Crank Nicolson*/
        ...
        long count = 0; //number of matrix inversions performed
        ...
        for(k=0;k<N_x;k++){
            for(m=0;m<N_y;m++){
                unknown_idx = indx2(k, m, N_y);
                if(is_valid(k-1, m, N_x, N_y)){
                    setv(&bmat, unknown_idx, indx2(k-1, m, N_y), -alpha);
                }
                if(is_valid(k+1, m, N_x, N_y)){
                    setv(&bmat, unknown_idx, indx2(k+1, m, N_y), -alpha);
                }
                if(is_valid(k, m-1, N_x, N_y)){
                    setv(&bmat, unknown_idx, indx2(k, m-1, N_y), -beta);
                }
                if(is_valid(k, m+1, N_x, N_y)){
                    setv(&bmat, unknown_idx, indx2(k, m+1, N_y), -beta);
                }
                setv(&bmat, unknown_idx, indx2(k, m, N_y), gamma);
        ...
        // x=0 Neumann BC
            ...
            for(m=0;m<N_y;m++){
                unknown_idx = indx2(0, m, N_y);
                setv(&bmat, unknown_idx, indx2(1, m, N_y), -2*alpha);
            }
        ...
        //x=L_x Neumann BC
            ...
            for(m=0;m<N_y;m++){
                unknown_idx = indx2(N_x-1, m, N_y);
                setv(&bmat, unknown_idx, indx2(N_x-2, m, N_y), -2*alpha);
            }
        ...
        // y=0 Neumann BC
            ...
            for(k=0;k<N_x;k++){
                unknown_idx = indx2(k, 0, N_y);
                setv(&bmat, unknown_idx, indx2(k, 1, N_y), -2*beta);
            }
        }
        ...
        // y=L_y Neumann BC
            ...
            for(k=0;k<N_x;k++){
                unknown_idx = indx2(k, N_y-1, N_y);
                setv(&bmat, k, indx2(k, N_y-2, N_y), -2*beta);
            }
        }
        ...
        while(ctime<t_f + dt/2){       
            ...
            for(k=0;k<N_x;k++){
                for(m=0;m<N_y;m++){
                    ...
                    b[unknown_idx] = uc[unknown_idx]*(1-2*alpha-2*beta);
                    if(is_valid(k-1, m, N_x, N_y)){
                        b[unknown_idx]+=alpha*uc[indx2(k-1, m, N_y)];
                    }
                    if(is_valid(k+1, m, N_x, N_y)){
                        b[unknown_idx]+=alpha*uc[indx2(k+1, m, N_y)];
                    }
                    if(is_valid(k, m-1, N_x, N_y)){
                        b[unknown_idx]+=beta*uc[indx2(k, m-1, N_y)];
                    }
                    if(is_valid(k, m+1, N_x, N_y)){
                        b[unknown_idx]+=beta*uc[indx2(k, m+1, N_y)];
                    }
...
//Checking that the value is in inside bmat so avoids negative value for bandno
int is_in_bmat(long i, long j, long bands){
